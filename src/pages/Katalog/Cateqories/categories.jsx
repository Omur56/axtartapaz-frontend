    ...
    >
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-28 sm:pb-12">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10 sm:mb-14">
          ...
          <p
            className={`mt-3 text-sm sm:text-base max-w-xl mx-auto leading-6 ${
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 sm:gap-5 lg:gap-6">
          {categories.map(({ id, path, label, icon, gradient, glow }) => (
            <Link
              key={id}
              to={`/katalog/${path}`}
              className={`group relative overflow-hidden rounded-3xl border p-[1px] transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl ${glow} ${
                darkMode
            ...
            >
              {/* Gradient top layer */}
              <div
                className={`relative flex min-h-[190px] sm:min-h-[215px] flex-col items-center justify-center overflow-hidden rounded-[23px] bg-gradient-to-br ${gradient} p-5`}
              >
                {/* Decorative circles */}
                <div className="absolute -right-8 -top-8 w-28 h-28 rounded-full bg-white/10 transition-transform duration-500 group-hover:scale-125" />

                <div className="absolute -left-10 -bottom-10 w-32 h-32 rounded-full bg-black/5 transition-transform duration-500 group-hover:scale-125" />

                {/* Icon container */}
                <div className="relative z-10 flex w-20 h-20 sm:w-24 sm:h-24 items-center justify-center rounded-3xl bg-white/90 shadow-xl backdrop-blur-sm transition-all duration-300 group-hover:scale-110 group-hover:rotate-2">
                  <img
                    src={icon}
                    alt={label}
                ...
                {/* Label */}
                <div className="relative z-10 flex items-center gap-1.5 mt-5 text-center">
                  <span className="text-sm sm:text-base font-extrabold text-white drop-shadow-sm leading-5">
                    {label}
                  </span>

                  <ArrowRight
                    size={16}
                    className="text-white opacity-0 -translate-x-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0 shrink-0"
                  />
                </div>
        ...
        {/* Bottom info */}
        <div
          className={`mt-10 sm:mt-14 rounded-3xl border p-5 sm:p-6 ${
            darkMode
        ...
        >
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 text-center">
            <div className="w-11 h-11 rounded-2xl bg-[#670fff]/10 text-[#670fff] flex items-center justify-center">
              <Grid3X3 size={21} />
            </div>
