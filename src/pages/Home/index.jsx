...
const Home = () => {
  ...
  const getCurrentTime = (d) =>
    ...
    new Date(d).toTimeString().slice(0, 5);
...
const SkeletonCard = () => {
  ...
  : visibleAds.map((item) => (
      <div key={item._id} className="relative">
        <Link
          target="_blank"
          to={`/${item.__type}/${item._id}`}
        >
          <div className="bg-gray-50 w-[185.7px] mb-10 h-[268.6px] sm:w-[268.75px] sm:h-[268.6px] max-w-[240.4px] max-h-[368.8px] rounded-[12px] shadow-md hover:shadow-lg transition-shadow duration-300 ease-in-out overflow-hidden">
